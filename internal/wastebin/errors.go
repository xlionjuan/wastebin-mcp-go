...
var (
	...
	errContentTooLarge           = errors.New("content exceeds the maximum allowed size; split the content into smaller parts and upload each separately")
	errServerRejected            = errors.New("server rejected the request; content may contain disallowed data")
	errContentTooLargeServer     = errors.New("content exceeds the server's maximum allowed size; split the content into smaller parts and upload each separately")
	errUnknownHTTP               = errors.New("unknown HTTP error")
	errFileNotText               = errors.New("file is binary or not valid UTF-8 text and cannot be uploaded. Do not attempt again for this file")
...
var (
	errPathTraversal           = errors.New("path traversal is not allowed and will always be rejected. Do not attempt again")
	errPathNotAllowed          = errors.New("file path is not under any configured allowed path; ask the user to check ALLOWED_PATHS if this path should be accessible")
	errBuiltinBlockedPrefix    = errors.New("file path is in a blocked system directory and will always be rejected. Do not attempt again")
	errBuiltinBlockedComponent = errors.New("file path contains a blocked component and will always be rejected. Do not attempt again")
	errUserBlockedPath         = errors.New("file path is in a user-blocked directory and will always be rejected. Do not attempt again")
	errFilePathCannotBeUsed    = errors.New("file path cannot be used")
	errPathNotFound            = errors.New("the specified file does not exist; verify the path is correct and do not attempt the same path again")
...
var (
	errFileReadDisabled           = errors.New("file read is disabled by configuration; use inline content instead. Do not attempt again")
	errInvalidExtension           = errors.New("extension contains invalid characters (/, \\, ?, #); use a plain extension like 'go' or 'py'")
	errSandboxTranslationNoMounts = errors.New("sandbox path translation was requested but no sandbox mounts are configured; ask the user to check WASTEBIN_MCP_SANDBOX_MOUNTS if translation should be enabled")
	errSandboxTranslationNoMatch  = errors.New("sandbox path does not match any configured mount; ask the user to check the sandbox mount configuration")
)
