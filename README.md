...
- All paths are resolved via `filepath.EvalSymlinks` before checking,
   preventing symlink-based bypass. Blocked-path entries are also resolved
   via `EvalSymlinks` at startup (with `Abs`+`Clean` fallback for
   non-existent paths), so symlink aliases cannot evade the blocklist.
